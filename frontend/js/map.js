...
/** Create a custom blue marker for the user location */
...
/** Setup geolocation tracking using Leaflet's built-in locate */
function setupGeolocation() {
  ...
  map.on('locationfound', (e) => {
    const pos = e.latlng;
    if (!userMarker) {
      userMarker = L.marker(pos, { icon: createUserMarkerIcon() }).addTo(map);
    } else {
  ...
  map.on('locationerror', (e) => {
    ...
    // Only alert if the user explicitly clicked the location button and we still have no position
    // (We'll check this in the button handler)
  });
  ...
  // Wire top buttons
  const userBtn = document.getElementById('btn-center-user');
  const restaurantsBtn = document.getElementById('btn-center-restaurants');
  ...
  if (userBtn) {
    userBtn.addEventListener('click', () => {
      if (userMarker) {
        ...
        // If marker is missing, try to locate again and show specific guidance
        map.locate({ setView: true, maxZoom: 18 });
        alert('Đang xác định vị trí của bạn... Vui lòng đảm bảo bạn đã cấp quyền truy cập vị trí và đang sử dụng kết nối bảo mật (HTTPS).');
      }
    });
  }
  ...
  if (restaurantsBtn) {
    restaurantsBtn.addEventListener('click', () => {
      if (restaurantData && restaurantData.length > 0) {
        ...
        alert('Đang tải dữ liệu quán ăn...');
      }
    });
  }
}

/** Fit map bounds to show the default street area */
function centerOnRestaurants() {
  map.flyTo(VK_CENTER, 17, { duration: 1.5 });
}
