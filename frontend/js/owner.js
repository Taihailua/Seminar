...
function openScannerModal() {
  ...
  `;
  modal.innerHTML = `
    <div style="background:#181828;border-radius:16px;padding:24px;width:90%;max-width:400px;border:1px solid #242437;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
        <h3 style="color:#0ea5e9;font-family:'Plus Jakarta Sans',sans-serif;margin:0;display:flex;align-items:center;gap:8px;">
          <span class="material-symbols-outlined notranslate">qr_code_scanner</span> Quét Mã QR
        </h3>
        <button id="close-scanner" style="background:none;border:none;color:#aba9bb;font-size:24px;cursor:pointer;">✕</button>
      </div>
      <div id="qr-reader" style="border-radius:12px;overflow:hidden;background:#000;"></div>
      <p style="color:#aba9bb;font-size:13px;text-align:center;margin-top:16px;">
        Quét mã QR của nhà hàng hoặc mã xác nhận của khách
      </p>
    </div>
  `;
  ...
  html5QrCode.start(
    ...
    (decodedText) => {
      html5QrCode.stop().then(() => {
        ...
        try {
          // If it's a URL in our system, navigate to it
